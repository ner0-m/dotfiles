...
globalkeys = gears.table.join(
    ...
    -- Focus client by direction (vim like keys)
    ...
    end, {description = "show notifications", group = "awesome"}),
    ...
    awful.key({modkey, shift}, "t", function()
        awful.spawn.easy_async_with_shell("slop -b 2 -c '0.61,0.9,0.75,1' -p -2", function(out)
            ...
            awful.spawn(terminal, {geometry = {x = myx, y = myy, height = myheight, width = mywidth}, floating = true})
        end)
    end, {description = "open a terminal", group = "launcher"}),
    awful.key({modkey}, "s", function()
        awful.spawn(music)
    end, {description = "open spot-tui", group = "launcher"}),
    ...
    awful.key({modkey, "Shift"}, "ö", function()
    ...
    -- increase/decrease number of width of focused window
    awful.key({modkey, "Control"}, "ö", function()
...
clientkeys = gears.table.join(awful.key({modkey, "Shift"}, "f", function(c)
    ...
    c:move_to_screen()
end, {description = "move to screen", group = "client"}), --  awful.key({ modkey, "Shift"   }, "t",      function (c) c.ontop = not c.ontop            end,
--            {description = "toggle keep on top", group = "client"}),
    ...
    c:raise()
end, {description = "(un)maximize horizontally", group = "client"}), -- On the fly useless gaps change
